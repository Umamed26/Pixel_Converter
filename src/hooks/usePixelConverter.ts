 ...
 * Trigger a Blob-backed download and release the object URL slightly later so bulk downloads keep the first file alive.
...
/**
 * 清洗下载文件名中的非法字符。/ Sanitize illegal characters from a download filename.
 * @param filename 原始文件名 / Raw filename.
 * @returns 安全文件名 / Safe filename.
 */
...
/**
 * 确保文件名带有期望扩展名。/ Ensure a filename keeps the expected extension.
 * @param filename 文件名 / Filename.
 * @param extension 扩展名（含点）/ Extension including dot.
 * @returns 规范化文件名 / Normalized filename.
 */
...
/**
 * 为重复文件名追加序号，避免导出时互相覆盖。/ Append numeric suffixes to avoid duplicate export filenames.
 * @param filename 原始文件名 / Original filename.
 * @param usedNames 已占用文件名集合 / Set of already used filenames.
 * @returns 唯一文件名 / Unique filename.
 */
function createUniqueFilename(filename: string, usedNames: Set<string>): string {
  const baseName = stripExt(filename) || "file";
...
export function usePixelConverter(ghostSrc: string) {
  ...
  const onExportPalette = useCallback(() => {
    ...
    downloadBlobAsFile(blob, `${palette}-palette.json`);
  ...
  const exportPresets = useCallback(() => {
    ...
    downloadBlobAsFile(blob, "pixel-converter-presets.json");
  }, [presets]);
  ...
  const processBatchFiles = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      if (completed > 0) {
        ...
        downloadBlobAsFile(zipBlob, `pixel-batch-${Date.now()}.zip`);
      }
  ...
  const downloadGalleryItem = useCallback((imageId: string) => {
    ...
    const baseName = sanitizeFilename(target?.name.trim() || `pixel-art-${Date.now()}`) || `pixel-art-${Date.now()}`;
    const fileName = ensureFilenameExtension(baseName, ".png");
  ...
  const downloadGalleryItemsBulk = useCallback(async (imageIds: string[]) => {
    ...
    try {
      ...
      const galleryItemMap = new Map(galleryItems.map((item) => [item.id, item]));
      let added = 0;
      for (let index = 0; index < ids.length; index += 1) {
        const id = ids[index];
        const blob = galleryBlobMapRef.current.get(id);
        if (!blob) {
          continue;
        }
        const item = galleryItemMap.get(id);
        const rawName = sanitizeFilename(item?.name.trim() || `pixel-art-${String(index + 1).padStart(3, "0")}`)
          || `pixel-art-${String(index + 1).padStart(3, "0")}`;
        const filename = createUniqueFilename(ensureFilenameExtension(rawName, ".png"), usedNames);
        zip.file(filename, blob);
        added += 1;
      }
      ...
      downloadBlobFile(zipBlob, `pixel-gallery-${Date.now()}.zip`);
      setStatusKey("statusDone");
    } catch {
      setStatusKey("statusReady");
    }
  ...
  const onExportProject = useCallback(() => {
    ...
    downloadBlobAsFile(blob, `pixel-project-${Date.now()}.pxc`);
  ...
  const onExportJson = useCallback(() => {
    ...
    downloadBlobAsFile(blob, "pixel-art.json");
  }, [grid, palette]);
